...
from models.conv import Conv, UpconvBilinear
...
class CDNet(nn.Module):
    def __init__(self):
        ...
        # Contour prediction
        self.cont = ContourPrediction(64)
    ...
    def forward(self, x):
        ...
        cont = self.cont(i2)
        upcont = F.interpolate(cont, scale_factor=2, mode='bilinear', align_corners=True)

        up1 = self.upconv1(i2)
        cat1 = torch.cat([up1, updisp2, upcont], dim=1)
        i1 = self.iconv1(cat1)
        disp1 = self.disp1(i1)

        return disp1, disp2, disp3, disp4, cont
