...
from models.cdnet9.conv import Conv, Upconv
from models.cdnet9.attention_modules import CBAM
...
class DenseBlock(nn.Module):
    def __init__(self, in_channels, growth_rate, num_layer, dropout_last=False):
        super().__init__()
        self.layers = nn.Sequential()
...
class TransitionLayer(nn.Module):
    def __init__(self, in_channels, out_channels, dropout_last=False):
        super().__init__()
        self.bn = nn.BatchNorm2d(in_channels)
        self.act = nn.ELU(inplace=True)
        self.conv = nn.Conv2d(in_channels, out_channels, 1, 1, 0)
        self.dropout = nn.Dropout2d() if dropout_last else nn.Sequential()
...
class CDNet9(nn.Module):
    def __init__(self):
        ...
        self.dense_block4 = DenseBlock(in_channels, growth_rate, num_layer, dropout_last=True)
        in_channels += num_layer * growth_rate
        out_channels_list.append(in_channels)
        # ----------------------------------------------------------------------

        # ---------------------------- Decoder ---------------------------------
        self.upconv5 = Upconv(out_channels_list[-1], out_channels_list[-2])
        self.upconv4 = Upconv(out_channels_list[-2], out_channels_list[-3])
        self.upconv3 = Upconv(out_channels_list[-3], out_channels_list[-4])
        self.upconv2 = Upconv(out_channels_list[-4], out_channels_list[-5])
        self.upconv1 = Upconv(out_channels_list[-5], out_channels_list[-6])
