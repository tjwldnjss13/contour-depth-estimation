...
class DNet5(nn.Module):
    def __init__(self):
        ...
        self.iconv1 = Conv(2 * out_channels_list[-6] + 2, out_channels_list[-6], 3, 1, 1)
        # ----------------------------------------------------------------------
    ...
    def forward(self, x):
        ...
        x3 = x = self.transition1(x)
        x = self.avgpool(x)

        x = self.dense_block2(x)
        x4 = x = self.transition2(x)
        x = self.avgpool(x)

        x = self.dense_block3(x)
        x5 = x = self.transition3(x)
        x = self.avgpool(x)
        ...
        up1 = self.upconv1(i2)
        cat1 = torch.cat([up1, x1, updisp2], dim=1)
        i1 = self.iconv1(cat1)
        disp1 = self.disp1(i1)

        return disp1, disp2, disp3, disp4

    def loss(self, image_left, image_right, disparities):
