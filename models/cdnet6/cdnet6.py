...
from models.cdnet6.conv import Conv, Upconv
from models.cdnet6.attention_modules import CBAM
...
class SRBlock1(nn.Module):
    def __init__(self, in_channels):
        super().__init__()
        self.conv1 = residual_13_layers(in_channels, 3, False)
        self.conv2 = nn.Sequential(
            Conv(in_channels, 2*in_channels, 3, 2, 1),
            residual_13_layers(2*in_channels, 2),
            Upconv(2*in_channels, in_channels, use_activation=False)
        )
        self.conv3 = nn.Sequential(
            Conv(in_channels, 2*in_channels, 3, 2, 1),
            Conv(2*in_channels, 4*in_channels, 3, 2, 1),
            residual_13_layers(4*in_channels, 2),
