...
from models.cdnet5.dnet5 import DNet5
...
def main(args):
    ...
    for e in range(args.epoch):
        ...
        for i, ann in enumerate(train_loader):
            ...
            disp1, disp2, disp3, disp4 = model(imgl)
            optimizer.zero_grad()
            loss, loss_ap, loss_ds, loss_lr = model.loss(imgl, imgr, [disp1, disp2, disp3, disp4])
            loss.backward()
            optimizer.step()
            ...
            del imgl, imgr, disp1, disp2, disp3, disp4, loss, loss_ap, loss_ds, loss_lr
        ...
        if not args.debug and (e + 1) % 1 == 0:
            save_pth = f'./save/{optim_name}_{model_name}_{dset_name}_{e+1}epoch_{cur_lr:.6f}lr_{train_loss:.5f}loss(train)_{val_loss:.5f}loss_{val_loss_ap:.5f}loss(ap)_{val_loss_ds:.5f}loss(ds)_{val_loss_lr:.5f}loss(lr).ckpt'
        ...
        if not args.debug and args.save_graph:
            plt.savefig(f'./graph/loss_{model_name}_{date_str}_{time_str}.png')
        plt.close()
